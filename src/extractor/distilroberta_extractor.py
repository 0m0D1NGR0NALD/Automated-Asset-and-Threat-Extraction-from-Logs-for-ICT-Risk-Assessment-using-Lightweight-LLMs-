...
class DistilRoBERTaExtractor(BaseExtractor):
    def __init__(self, asset_labels=None, threat_labels=None):
        ...
        # Use zero-shot classification (DistilRoBERTa MNLI)
        self.classifier = pipeline(
            "zero-shot-classification",
            model="typeform/distilbert-base-uncased-mnli",
            device=self.device
        )
        self.asset_labels = asset_labels or [
            "public_web_server", "internal_database", "developer_workstation",
            "test_environment", "unknown"
        ]
        self.threat_labels = threat_labels or [
            "sql_injection", "xss", "dos", "brute_force",
            "privilege_escalation", "malicious_file", "info_leak"
        ]
